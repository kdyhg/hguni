...
import { reportBridgeJob } from "@/lib/server/local-bridge";
...
export async function POST(request:Request,context:{params:Promise<{id:string}>}) {
  try {
    ...
    if(!isMockMode()) reportBridgeJob(id, bridge.id, input.leaseGeneration, { outcome: input.outcome, result: input.result });
